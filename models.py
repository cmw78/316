...
class Student(db.Model):
  ...
  def addFriend(yourid,friendid):
    try:
      ...
      raise e
  @staticmethod
  def info(studentid, courseid, bookbagged,taking,taken):
    try:
      print studentid
      db.session.execute('DELETE FROM bookbag WHERE studentid=:studentid AND courseid=:courseid', dict(studentid=studentid, courseid=courseid))
      db.session.execute('DELETE FROM taking WHERE studentid=:studentid AND courseid=:courseid', dict(studentid=studentid, courseid=courseid))
      db.session.execute('DELETE FROM taken WHERE studentid=:studentid AND courseid=:courseid', dict(studentid=studentid, courseid=courseid))
